...
<html lang="en">
  ...
  <script>
    ...
    if (sessionStorage.getItem('isLoggedIn') !== 'true') {
        ...
        document.getElementById('content').textContent = 'You are logged in!';
    }
</script>
...
<style>
  ...
  <body>
    ...
    <div class="ticket-section">
      ...
      <button class="switchButton" onclick="location.href='HSPrinterCheck.php'" id='campusSwitch'> Switch to Harborside </button>
      <button class="submit" id="refreshButton" onclick="refresh()"> Refresh </button>
      <div id="loading">Loading...</div> <!-- Loading indicator -->

      <textarea id="full_send_output" placeholder="Output here" rows="4" cols="50" readonly></textarea>

      <div class="printer-container" name="worm_was_here">
  ...
  <script>
    ...
    function refresh() {
		...
		// Make a request to the URL with no-cors mode
		fetch('http://10.9.5.21:8080/full_send', {
			method: 'POST',
			mode: 'no-cors'
		})
		.then(response => {
        // Since the response is opaque, you won't be able to read the response data
			document.getElementById('full_send_output').value = 'Request sent successfully, but response is opaque.';
			loadingIndicator.style.display = 'none';
		})
		.catch(error => {
			document.getElementById('full_send_output').value = `Full send failed: ${error}`;
			loadingIndicator.style.display = 'none';
		});
	}
